...
// removed emailjs import (no third-party needed)
...
const ContactSection = () => {
  ...
  // NEW: no keys, no service — just open the user's mail client with your email prefilled
  const sendEmail = (e) => {
    e.preventDefault();
    setLoading(true);
    ...
    // compose mailto
    const to = 'ahmadrza.1110@gmail.com';
    const finalSubject = `[Portfolio] ${subject}`;
    const body =
      `From: ${name} <${email}>\n\n` +
      `${message}\n\n` +
      `Sent: ${formattedDate(new Date())}`;
    ...
    // done – remove loader; optionally reset the form
    setTimeout(() => {
      setLoading(false);
      try {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="flex flex-col justify-between order-2 md:order-1">
          <p className="my-6 cursor-pointer cursor-white cursor-scale-1.2">
            Open to <span className="text-primary">full-time</span> or <span className="text-primary">long-term contract</span> roles (onsite or remote) in <span className="text-primary">Front-End, Web Development,</span> or <span className="text-primary">AI Chatbot</span> engineering. I’m <span className="text-primary">not available for freelance work</span>. If you have a suitable opportunity or any questions, feel free to contact me.
          </p>
          <div>
            <Floating>
